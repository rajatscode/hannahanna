...
"#,
    ...
    assert!(
        marker.exists(),
        "pre_create hook should have created marker"
    );

    // Post-create hook should have run in worktree directory
    let wt_path = repo.worktree_path("test-wt");
    assert!(
        wt_path.join("post_create.txt").exists(),
        "post_create hook should have created file"
    );
}
...
"#,
    ...
    assert!(repo
        .worktree_path("feature-hotfix-123")
        .join("hotfix.txt")
        .exists());
}
...
"#,
    ...
    repo.hn(&["add", "no-hooks-test", "--no-hooks"])
        .assert_success();
...
"#,
    ...
    let content =
        fs::read_to_string(repo.worktree_path("shell-var-test").join("shell_var.txt")).unwrap();
...
"#,
    ...
    repo.hn(&["add", "test-branch", "--no-hooks"])
        .assert_success();
