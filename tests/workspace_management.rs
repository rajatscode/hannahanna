...
fn test_workspace_save_duplicate_name_fails() {
    ...
    repo.hn(&["workspace", "save", "duplicate"])
        .assert_success();
...
fn test_workspace_restore_basic() {
    ...
    repo.hn(&["workspace", "save", "restore-test"])
        .assert_success();

    // Remove worktrees
    repo.hn(&["remove", "restore-wt1", "--force"])
        .assert_success();
    repo.hn(&["remove", "restore-wt2", "--force"])
        .assert_success();
    ...
    assert!(
        result.success,
        "Workspace restore should succeed: {}",
        result.stderr
    );
    ...
    assert!(
        list_result.stdout.contains("restore-wt1"),
        "restore-wt1 not in list output"
    );
    assert!(
        list_result.stdout.contains("restore-wt2"),
        "restore-wt2 not in list output"
    );
}
...
fn test_workspace_restore_with_conflicts() {
    ...
    repo.hn(&["workspace", "save", "conflict-test"])
        .assert_success();
...
fn test_workspace_restore_force() {
    ...
    repo.hn(&["workspace", "save", "force-test"])
        .assert_success();
...
fn test_workspace_list_with_saved_workspaces() {
    ...
    repo.hn(&["add", "wt1"]).assert_success();
    repo.hn(&["workspace", "save", "workspace1"])
        .assert_success();

    repo.hn(&["add", "wt2"]).assert_success();
    repo.hn(&["workspace", "save", "workspace2"])
        .assert_success();
...
fn test_workspace_list_json_output() {
    ...
    repo.hn(&["add", "wt1"]).assert_success();
    repo.hn(&["workspace", "save", "json-test"])
        .assert_success();
...
fn test_workspace_list_shows_metadata() {
    ...
    repo.hn(&["workspace", "save", "meta-workspace"])
        .assert_success();
...
fn test_workspace_delete_basic() {
    ...
    repo.hn(&["workspace", "save", "delete-me"])
        .assert_success();
...
fn test_workspace_delete_confirmation() {
    ...
    repo.hn(&["workspace", "save", "confirm-delete"])
        .assert_success();
...
fn test_workspace_delete_doesnt_remove_worktrees() {
    ...
    repo.hn(&["workspace", "save", "persist-workspace"])
        .assert_success();

    repo.hn(&["workspace", "delete", "persist-workspace", "--force"])
        .assert_success();
...
fn test_workspace_full_lifecycle() {
    ...
    // 2. Save workspace
    repo.hn(&["workspace", "save", "lifecycle"])
        .assert_success();
    ...
    // 4. Remove worktrees
    repo.hn(&["remove", "lifecycle-wt1", "--force"])
        .assert_success();
    repo.hn(&["remove", "lifecycle-wt2", "--force"])
        .assert_success();

    // 5. Restore workspace
    repo.hn(&["workspace", "restore", "lifecycle"])
        .assert_success();
    ...
    // 7. Delete workspace
    repo.hn(&["workspace", "delete", "lifecycle", "--force"])
        .assert_success();
...
"#,
    ...
    repo.hn(&["workspace", "save", "with-state"])
        .assert_success();
}
...
fn test_workspace_restore_preserves_branches() {
    ...
    repo.hn(&["remove", "branch-wt1", "--force"])
        .assert_success();
    repo.hn(&["remove", "branch-wt2", "--force"])
        .assert_success();

    repo.hn(&["workspace", "restore", "branches"])
        .assert_success();
...
fn test_workspace_multiple_saves() {
    ...
    repo.hn(&["workspace", "save", "version-1"])
        .assert_success();

    repo.hn(&["add", "v2-wt"]).assert_success();
    repo.hn(&["workspace", "save", "version-2"])
        .assert_success();

    repo.hn(&["add", "v3-wt"]).assert_success();
    repo.hn(&["workspace", "save", "version-3"])
        .assert_success();
...
fn test_workspace_restore_partial_success() {
    ...
    repo.hn(&["remove", "partial-wt1", "--force"])
        .assert_success();
...
"#,
    ...
    repo.hn(&["workspace", "save", "with-config"])
        .assert_success();
...
fn test_workspace_name_validation() {
    ...
    for name in invalid_names {
        ...
        assert!(
            !result.success,
            "Name '{}' should fail validation but succeeded",
            name
        );
    }
}
...
fn test_workspace_export_import() {
    ...
    repo.hn(&["workspace", "save", "export-test"])
        .assert_success();

    // If export functionality exists
    let _result = repo.hn(&["workspace", "export", "export-test"]);
