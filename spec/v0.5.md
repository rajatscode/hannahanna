...
v0.5 builds on v0.4's performance foundation by delivering the deferred template management features, enhanced workflow capabilities, and comprehensive testing improvements. This release focuses on user-facing features that improve the daily development workflow without introducing breaking changes.

**Key Principle:** Incremental value delivery. Ship features that users are asking for while maintaining backward compatibility and high quality standards.

---

## What's In Scope for v0.5

### 0. 🐛 Critical Fix: Environment Variable Naming (Highest Priority)
...
**Implementation Strategy (Backward Compatible):**

**Phase 1 (v0.5.0):** Dual support with deprecation warning
```rust
// Set BOTH old and new variables
env.insert("HNHN_NAME".to_string(), worktree.name.clone());
env.insert("WT_NAME".to_string(), worktree.name.clone()); // Deprecated

// Add deprecation warning to hook output
if hook_uses_wt_vars(&script) {
    eprintln!("⚠️  Warning: Hook uses deprecated WT_* environment variables");
    eprintln!("   Please update to HNHN_* variables. WT_* will be removed in v1.0");
    eprintln!("   See: docs/MIGRATING.md#v05-environment-variables");
}
```

**Phase 2 (v1.0.0):** Remove old variables
- Only set HNHN_* variables
- Breaking change documented in v1.0 migration guide
...
**Tests to add:**
- Verify both HNHN_* and WT_* variables are set in v0.5
- Verify hook execution with both variable sets
- Verify deprecation warning appears when WT_* is used

**Migration Guide Entry:**
```markdown
## v0.5.0: Environment Variable Renaming
...
**Action Required:**
Update your hooks to use the new variable names:
- `$WT_NAME` → `$HNHN_NAME`
- `$WT_PATH` → `$HNHN_PATH`
...
v0.5 sets BOTH old and new variables, so existing hooks will continue to work.
However, old variables will be removed in v1.0, so please update your hooks.

**Example:**
```bash
# Old (deprecated, works in v0.5, removed in v1.0)
...
**Acceptance Criteria:**
- [ ] All hooks receive both HNHN_* and WT_* variables
- [ ] Deprecation warning shown when WT_* variables detected in use
- [ ] All documentation updated to show HNHN_* variables
- [ ] Migration guide entry added
- [ ] 5+ tests verifying both variable sets
...
#### `hn templates list`
**Status:** 🟢 Infrastructure exists (src/templates.rs has `list_templates()`)
...
Available Templates
═══════════════════════════════════════════════════════════
microservice   │ Backend microservice with Docker
frontend       │ React/Vue frontend setup
experiment     │ Minimal config for quick experiments
═══════════════════════════════════════════════════════════
...
├── .hannahanna.yml       # Config overrides
├── README.md             # Template description
└── files/                # Files to copy (NEW)
    ├── .env.example
...
### 2. 🧪 Comprehensive Hook & Docker Testing (High Priority)
...
### 3. 💾 Workflow State Management (Medium Priority)
...
### 4. 📊 Resource Usage Tracking (Low Priority)
...
**Day 1:** Environment variable renaming (HN_* migration)
- Update src/hooks.rs
...
**Days 2-3:** `hn templates list` and `hn templates show`
- Implement CLI commands
- Use existing list_templates() infrastructure
- Add tests

**Days 4-6:** `hn templates create`
- Interactive wizard
- Create from scratch or current worktree
- README generation
- Tests

**Days 7-9:** Template file copying
...
**Days 16-19:** Workspace state management
- Save/restore commands
- Docker integration
- Tests

**Days 20-22:** Resource tracking
- Stats command
...
### Low Risk
- ✅ Template listing (infrastructure exists)
- ✅ Additional testing (pure addition)
- ✅ Resource tracking (isolated feature)

### Medium Risk
- ⚠️ Template file copying (file I/O, permissions)
  - **Mitigation:** Comprehensive testing, security validation
- ⚠️ Workspace restore (Docker orchestration complexity)
  - **Mitigation:** Graceful failure handling, dry-run mode

### High Risk
- ❌ None identified for v0.5 scope
...
- ✅ No shortcuts: Complete features properly
- ✅ Fix all bugs: Comprehensive testing added
- ✅ Measure performance: Maintain benchmark suite
- ✅ Accurate docs: Documentation is part of every feature
...
**Ready to build?** Let's start with template management! 🚀
