...
#### Setup Command
- **What**: Automated installation and configuration command
- **Usage**:
  ```bash
  hn setup                # Auto-detect shell
  hn setup --shell bash   # Specify shell
  ```
- **Implementation**: `src/cli/setup.rs` (308 lines)
- **Features**:
  - Shell detection (bash/zsh/fish)
  - Completion installation to shell-specific paths
  - Shell integration setup with instructions
  - Example template creation (microservice, frontend)
  - Environment validation (git, docker)

#### Enhanced `hn info` Output
- **What**: Rich, colorful display with actionable information
- **New Fields**:
  - VCS type display
  - Status with emoji (✓ clean / ⚠ dirty)
...
#### Test Coverage
- **Total Tests**: 273 tests (87 lib + 186 integration)
- **New v0.4 Tests**:
...
- **`docs/MIGRATING.md`**: v0.3 → v0.4 migration guide
...
|-----------|--------|-----------------|--------|
| List 100 worktrees (no cache) | < 2.5s | 2.09s | ✅ PASS |
| List 50 worktrees (cache hit) | < 100ms | ~50ms | ✅ PASS |
| Create worktree | < 500ms | ~300ms | ✅ PASS |
| Fuzzy search 1000 items | < 10ms | ~5ms | ✅ PASS |
| Port allocation (10 concurrent) | < 2s | ~800ms | ✅ PASS |
| Config load | < 50ms | ~15ms | ✅ PASS |
...
- New `hn setup [--shell <shell>]` command
...
3. (Recommended) Run `hn setup` for completions and templates
   - Or manually: `hn completions <shell>` and create `.hn-templates/`
